...
$IVA=consultasSQL::clean_string($_POST['valorIVA']);
  ...
  if(!empty($_SESSION['carro'])){  
    ...
    foreach($_SESSION['carro'] as $codess){
        ...
        while($fila = mysqli_fetch_array($consulta, MYSQLI_ASSOC)) {
          ...
          $suma += $tp*$codess['cantidad']+$envio;
        }
        mysqli_free_result($consulta);
    }
    if(consultasSQL::InsertSQL("venta", "Fecha, NIT, TotalPagar, ValorIVA, Estado, NumeroDeposito, TipoEnvio, Adjunto", "'".date('d-m-Y')."','$Cedclien','$suma','$IVA','$StatusV','$NumDepo','$tipoenvio','$comprobanteF'")){
