...
if ($nombre != "" && $clave != "") {
    ...
    $verActivo = ejecutarSQL::consultar("SELECT * FROM cliente WHERE Nombre='$nombre' AND Clave='$clave' AND Activado='si'");
    $filaU1 = mysqli_fetch_array($verActivo, MYSQLI_ASSOC);
    $UserC1 = mysqli_num_rows($verActivo);

    if ($UserC1 > 0) {
        $_SESSION['nombreUser'] = $nombre;
        $_SESSION['claveUser'] = $clave;
        $_SESSION['UserType'] = "User";
        $_SESSION['UserNIT'] = $filaU1['NIT'];
    ...
    echo 'Tu cuenta no ha sido verificada, por favor revisa tu correo electrónico';
} 
} else {
